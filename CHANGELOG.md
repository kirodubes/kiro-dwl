...
**Technical Details**
- `grp:alt_shift_toggle` matches the CachyOS Calamares reference (`keyboard/Config.cpp` defaults the group switcher to it when a second layout exists). `compose:caps` (Caps = Compose) unchanged. `xkb_rule_names` in `config.h` — **baked into the dwl binary, so the rebuild recompiles**. Skel copy + repo-root copy kept in lockstep.

**Files Modified**
- `etc/skel/.config/dwl/config.h`
- `config.h` (repo-root build copy)
